...
#   3) embed
#   4) RAG
#   5) dataset fallback only if RAG has no context
#   6) GPT
# ─────────────────────────────────────────────────────────────────────────────
def generate_answer(q: str, history=None):
    ...
    if is_out_of_scope(q) and not has_dental_signal(q) and not history_has_dental_context(history or []):
        return {
    ...
    if off_topic and not has_dental_signal(q) and not history_has_dental_context(history or []):
        return {
            "answer": refusal_scope(q),
            "refs": [],
            "source": "scope_refusal",
